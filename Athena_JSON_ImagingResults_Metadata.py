...
def extract_data_from_json(json_data):
    extracted_data = []
    # Ensure the JSON data contains an 'ImagingResult' array
    ImagingResult = json_data.get('imagingresults', [])
    for i, doc in enumerate(ImagingResult):
        try:
            ...
            imagingresultid = doc.get('imagingresultid', '')
            observationdate = doc.get('observationdate', '')
            ordertype = doc.get('ordertype', '')
            extracted_data.append([createddate, description, documentsource, imagingresultid, observationdate, ordertype])
...
def write_json_to_csv(json_directory, csv_file_path):
    ...
    try:
        with open(csv_file_path, 'w', newline='', encoding='utf-8') as csv_file:
            ...
            # Write header
            csv_writer.writerow(['createddate', 'description', 'documentsource', 'imagingresultid', 'observationdate', 'ordertype'])
            
            # Recursively search for 'ImagingResult.json' files
            for root, dirs, files in os.walk(json_directory):
                for filename in files:
                    if filename == 'ImagingResult.json':
